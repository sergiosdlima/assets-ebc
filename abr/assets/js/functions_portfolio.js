...
"use strict";
    ...
    var portfolioIsotope = function() {
        if ( $().isotope ) {
            ...
            var $container2 = $('.servicos-wrap');
            $container2.imagesLoaded(function(){
                $container2.isotope({
                    itemSelector: '.servicos-item',
                    transitionDuration: '0.5s',
                });
            });

            $('.text-produtos.nav-filter').on('click',function() {
                var selector = $(this).attr('data-filter');
                $('.nav-tabs .text-produtos.nav-filter').removeClass('active');
                $(this).addClass('active');
                $container.isotope({ filter: selector });
                return false;
            });
             $('.text-servicos.nav-filter').on('click',function() {
                var selector = $(this).attr('data-filter');
                $('.nav-tabs .text-servicos.nav-filter').removeClass('active');
                $(this).addClass('active');
